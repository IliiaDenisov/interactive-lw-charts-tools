...
export abstract class CollisionHelper {
  public static HitTestLineToLine(
    lineStart1: Point2D,
    lineEnd1: Point2D,
    lineStart2: Point2D,
    lineEnd2: Point2D
  ...
  public static HitTestLineToRect(
    lineStart1: Point2D,
    lineEnd1: Point2D,
    rect: Box
  ): [boolean, Point2D[]] {
    ...
    const rectPoints: Point2D[] = [
      new Point2D(rect.xmin, rect.ymin),
      new Point2D(rect.xmin, rect.ymax),
      new Point2D(rect.xmax, rect.ymax),
      new Point2D(rect.xmax, rect.ymin),
    ];
    ...
    for (let i = 0; i < 4; ++i) {
      ...
      [bHit, collisionPoint] = this.HitTestLineToLine(
        lineStart1,
        lineEnd1,
        lineStart2,
        lineEnd2
  ...
  public static HitTestQuadraticBezierCurve(
    p1: Point2D,
    p2: Point2D,
    ...
    const len: number =
      controlPoint.distanceTo(p1)[0] + controlPoint.distanceTo(p2)[0];
    ...
    for (let t = 0; t <= 1; t += step) {
      ...
      const currPointOnCurve = new Point2D(
        s1.x + s2.x + s3.x,
        s1.y + s2.y + s3.y
      );
      if (currPointOnCurve.distanceTo(point)[0] < tolerance) return true;
  ...
  public static IsPointInEllipse(
    point: Point2D,
    ellipseOrigin: Point2D,
    semiMajorAxis: number,
    semiMinorAxis: number
  ): boolean {
    if (
      MathHelper.AreEqual(semiMinorAxis, 0.0) ||
      MathHelper.AreEqual(semiMajorAxis, 0.0)
    )
      return false;

    return (
      ((point.x - ellipseOrigin.x) * (point.x - ellipseOrigin.x)) /
        (semiMajorAxis * semiMajorAxis) +
        ((point.y - ellipseOrigin.y) * (point.y - ellipseOrigin.y)) /
  ...
  public static IsPointInPolygon(
    ...
    for (i = 0; i < polygonPoints.length; i++) {
      if (
        ((polygonPoints[i].y < y && polygonPoints[j].y >= y) ||
  ...
  public static IsLineInRectangle(
    lineStart1: Point2D,
    lineEnd1: Point2D,
    rect: Box
  ): boolean {
    return (
      this.IsPointInRectangle(lineStart1, rect) &&
      this.IsPointInRectangle(lineEnd1, rect)
    );
  }

  public static IsPointInAreaEnclosedByCurve(
    point: Point2D,
    vertex1: Point2D,
    vertex2: Point2D,
    coVertex: Point2D
  ): boolean {
    const dir: Vector2D = new Vector2D(vertex1.x, vertex1.y).subtract(
      new Vector2D(vertex2.x, vertex2.y)
    );
    const controlPoint1 = new Vector2D(coVertex.x, coVertex.y).add(
      dir.scale(0.25, 0.25)
    );
    const controlPoint2 = new Vector2D(coVertex.x, coVertex.y).subtract(
      dir.scale(0.25, 0.25)
    );

    const firstHalfPoints: Point2D[] =
      MathHelper.ApproximateQuadraticBezierCurveWithPolyline(
        vertex1,
        coVertex,
        new Point2D(controlPoint1.x, controlPoint1.y)
      );
    const secondHalfPoints: Point2D[] =
      MathHelper.ApproximateQuadraticBezierCurveWithPolyline(
        coVertex,
        vertex2,
        new Point2D(controlPoint2.x, controlPoint2.y)
      );
  ...
  public static HitTestArc(
    ...
    arcStartAngleDir = MathHelper.RotateVector(
      arcStartAngleDir,
      MathHelper.ToRadians(arcStartAngle)
    );

    let currVecDir: Vector2D = new Vector2D(currPoint.x, currPoint.y).subtract(
      new Vector2D(arcCenter.x, arcCenter.y)
    );
    currVecDir = currVecDir.normalize();

    const angleDeviationFromStartAngle: number = MathHelper.ToDegrees(
      MathHelper.AngleBetweenVectors(arcStartAngleDir, currVecDir)
    );
    ...
    return bHits
      ? this.HitTestFullCircleArc(currPoint, arcCenter, arcRadius, tolerance)
      : false;
  }

  public static HitTestFullCircleArc(
    point: Point2D,
    arcCenter: Point2D,
    radius: number,
    tolerance: number
  ): boolean {
    return Math.abs(radius - point.distanceTo(arcCenter)[0]) < tolerance
      ? true
      : false;
  }
}
