...
export function fillAnnulusSector(
  renderingScope: BitmapCoordinatesRenderingScope,
  annulusRenderInfo: AnnulusSectorRenderInfo,
  fillColor: string,
  lineColor: string
) {
...
export function drawFibWedge(
  ...
  const startAngle = MathHelper.AngleBetweenVectors(
    new Vector2D(1.0, 0.0),
    trendLineVec1
  );
  const sweepAngle = MathHelper.AngleBetweenVectors(
    trendLineVec1,
    trendLineVec2
  );
  ...
  for (let i = 1; i < fibonacciLevels.length; i++) {
    ...
    angleBisectorVec = MathHelper.RotateVector(
      angleBisectorVec,
      sweepAngle / 2
    );
...
class FibWedgePaneRenderer implements IPrimitivePaneRenderer {
    ...
    target.useBitmapCoordinateSpace((scope) => {
      ...
      if (this._points.length < 3) {
        ...
        drawFibWedge(
          scope,
          points,
          this._fibonacciLevels,
          this._fibonacciFillColors,
          this._fibonacciLineColors
        );
      }
    });
  }
  ...
  hitTest(x: number, y: number): PrimitiveHoveredItem | null {
    ...
    const radiusLine1: Segment = new Segment(
      new Point2D(center.x, center.y),
      new Point2D(r1.x, r1.y)
    );
    const radiusLine2: Segment = new Segment(
      new Point2D(center.x, center.y),
      new Point2D(r2.x, r2.y)
    );
    ...
    const radius: number = new Point2D(r1.x, r1.y).distanceTo(
      new Point2D(center.x, center.y)
    )[0];

    for (let i = 1; i < this._fibonacciLevels.length && !hit; i++) {
      const level: number = this._fibonacciLevels[i];
      const startAngleDegrees: number = MathHelper.ToDegrees(
        MathHelper.AngleBetweenVectors(
          new Vector2D(1.0, 0.0),
          r1.subtract(center)
        )
      );
      const sweepClockwiseAngleDegrees: number = MathHelper.ToDegrees(
        MathHelper.AngleBetweenVectors(r1.subtract(center), r2.subtract(center))
      );
...
class FibWedge extends DrawingBase<FibWedgeOptions> {
  ...
  constructor(points: Point[], options: Partial<FibWedgeOptions> = {}) {
