...
import {
  ...
  convertViewPointToVector2D,
} from "./conversion-helper.ts";

export function fillBezierPath(
  renderingScope: BitmapCoordinatesRenderingScope,
  bezierCurveInfo: BezierCurvesPointsInfo,
  fillColor: string,
  lineColor: string
) {
  const ctx = renderingScope.context;

  const bezierSplines: Point2D[] = Array<Point2D>(
    bezierCurveInfo.endPoints.length +
...
class CurvePaneRenderer implements IPrimitivePaneRenderer {
  ...
  hitTest(x: number, y: number): PrimitiveHoveredItem | null {
    ...
    const hitFirstHalf = CollisionHelper.HitTestQuadraticBezierCurve(
      new Point2D(vertex1.x, vertex1.y),
      new Point2D(coVertex.x, coVertex.y),
      new Point2D(controlPoint1.x, controlPoint1.y),
      currPoint,
      epsilon
    );
    const hitSecondHalf = CollisionHelper.HitTestQuadraticBezierCurve(
      new Point2D(coVertex.x, coVertex.y),
...
class CurvePaneView implements IPrimitivePaneView {
  ...
  renderer() {
    return new CurvePaneRenderer(this._drawingPoints, this._source._options);
  }
}
...
class Curve extends DrawingBase<CurveOptions> {
  ...
  constructor(points: Point[], options: Partial<CurveOptions> = {}) {
