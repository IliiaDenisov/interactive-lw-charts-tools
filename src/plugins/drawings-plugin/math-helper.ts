...
export abstract class MathHelper {
  ...
  public static GetCubicBezierCurveDrawingPoints(
    vertex1: Point2D,
    apex: Point2D,
    vertex2: Point2D
  ): BezierCurvesPointsInfo {
    ...
    const controlPoint0_0 = p1.add(
      cp1.subtract(p1).scale(2.0 / 3.0, 2.0 / 3.0)
    );
    const controlPoint1_0 = p2.add(
      cp1.subtract(p2).scale(2.0 / 3.0, 2.0 / 3.0)
    );

    const controlPoint0_1 = p2.add(
      cp2.subtract(p2).scale(2.0 / 3.0, 2.0 / 3.0)
    );
    const controlPoint1_1 = p3.add(
      cp2.subtract(p3).scale(2.0 / 3.0, 2.0 / 3.0)
    );
    ...
    const controlPoints1: Point2D[] = [
      vectorToPoint(controlPoint0_0),
      vectorToPoint(controlPoint0_1),
    ];
    const controlPoints2: Point2D[] = [
      vectorToPoint(controlPoint1_0),
      vectorToPoint(controlPoint1_1),
    ];

    return { endPoints, controlPoints1, controlPoints2 };
  }

  public static ApproximateQuadraticBezierCurveWithPolyline(
    point1: Point2D,
    point2: Point2D,
    controlPoint: Point2D
  ) {
    const len: number = Math.round(
      controlPoint.distanceTo(point1)[0] + controlPoint.distanceTo(point2)[0]
    );
    ...
    for (let t = 0.0; t < 1.0; t += step) {
      ...
      // P1 = controlPoint
      // P2 = point2
      const s1: Vector2D = new Vector2D(point1.x, point1.y).scale(
        (1 - t) * (1 - t),
        (1 - t) * (1 - t)
      );
      const s2: Vector2D = new Vector2D(controlPoint.x, controlPoint.y).scale(
        2 * t * (1 - t),
        2 * t * (1 - t)
      );
