...
class FibSpiralPaneRenderer implements IPrimitivePaneRenderer {
  ...
  draw(target: CanvasRenderingTarget2D) {
    target.useBitmapCoordinateSpace((scope) => {
      if (this._fibSpiralRendeInfo.numArcs == 0) return;

      const degreesToRadian = (degrees: number): number => {
        return (degrees / 180.0) * Math.PI;
      };

      const rotationCenter = convertViewPointToPoint2D(
        calculateDrawingPoint(this._fibSpiralRendeInfo.rotationCenter, scope)
      );
      ...
      const rayStart = convertViewPointToPoint2D(
        calculateDrawingPoint(this._fibSpiralRendeInfo.rayStart, scope)
      );
      const rayEnd = convertViewPointToPoint2D(
        calculateDrawingPoint(this._fibSpiralRendeInfo.rayEnd, scope)
      );
      ...
      for (let i = 0; i < numArcs; ++i) {
        const center: Point2D = convertViewPointToPoint2D(
          calculateDrawingPoint(arcCenters[i], scope)
        );
  ...
  hitTest(x: number, y: number): PrimitiveHoveredItem | null {
    ...
    const rotationCenter: Vector2D = new Vector2D(
      this._fibSpiralRendeInfo.rotationCenter.x as number,
      this._fibSpiralRendeInfo.rotationCenter.y as number
    );

    // Rotate current point around fibSpiralDrawInfo.rotationCenter to -rotationAngle
    let hitTestPointLocal: Vector2D = new Vector2D(
      hitTestPoint.x,
      hitTestPoint.y
    ).subtract(rotationCenter);
    hitTestPointLocal = MathHelper.RotateVector(
      hitTestPointLocal,
      -this._fibSpiralRendeInfo.spiralRotationAngle
    );
    const hitTestPointRotated: Vector2D = rotationCenter.add(hitTestPointLocal);

    const ray: Segment = new Segment(
      new Point2D(
        this._fibSpiralRendeInfo.rayStart.x as number,
        this._fibSpiralRendeInfo.rayStart.y as number
      ),
      new Point2D(
        this._fibSpiralRendeInfo.rayEnd.x as number,
        this._fibSpiralRendeInfo.rayEnd.y as number
      )
    );

    let hit: boolean = false;
    const distToRay: number = ray.distanceTo(
    ...
    for (let i = 0; i < this._fibSpiralRendeInfo.numArcs && !hit; i++) {
      ...
      const arcCenterLocal: Vector2D = new Vector2D(
        this._fibSpiralRendeInfo.arcCenters[i].x as number,
        this._fibSpiralRendeInfo.arcCenters[i].y as number
      );
...
class FibSpiralPaneView implements IPrimitivePaneView {
  ...
  updateRenderInfo(drawingPoints: ViewPoint[]): FibSpiralRenderInfo {
    let spiralRotationCenter: ViewPoint = {
      x: 0 as Coordinate,
      y: 0 as Coordinate,
    };
    ...
    const pointsAreValid =
      ...
      drawingPoints[0].x != null &&
      drawingPoints[1].x != null &&
      drawingPoints[0].y != null &&
    ...
    if (pointsAreValid && !pointAreEqual) {
      const p1: Vector2D = new Vector2D(
        drawingPoints[0].x as number,
        drawingPoints[0].y as number
      );
      const p2: Vector2D = new Vector2D(
        drawingPoints[1].x as number,
        drawingPoints[1].y as number
      );

      let initDir = p2.subtract(p1);

      spiralRotationAngle =
        -1 * MathHelper.AngleBetweenVectors(initDir, new Vector2D(1, 0));
      ...
      arcCenters[0] = {
        x: 0.0 as Coordinate,
        y: (clockwiseCoef * a) as Coordinate,
      };
      arcCenters[1] = {
        x: -a as Coordinate,
        y: (clockwiseCoef * a) as Coordinate,
      ...
      arcCenters[2] = {
        x: -a as Coordinate,
        y: (clockwiseCoef * 0.0) as Coordinate,
      };
      arcCenters[3] = {
        x: a as Coordinate,
        y: (clockwiseCoef * 0.0) as Coordinate,
      ...
      arcCenters[4] = {
        x: a as Coordinate,
        y: (clockwiseCoef * 3.0 * a) as Coordinate,
      };
      arcCenters[5] = {
        x: (-4.0 * a) as Coordinate,
        y: (clockwiseCoef * 3.0 * a) as Coordinate,
      };
      arcCenters[6] = {
        x: (-4.0 * a) as Coordinate,
        y: (clockwiseCoef * -5.0 * a) as Coordinate,
      };
      arcCenters[7] = {
        x: (9.0 * a) as Coordinate,
        y: (clockwiseCoef * -5.0 * a) as Coordinate,
      };
      arcCenters[8] = {
        x: (9.0 * a) as Coordinate,
        y: (clockwiseCoef * 16.0 * a) as Coordinate,
      };
      arcCenters[9] = {
        x: (-25.0 * a) as Coordinate,
        y: (clockwiseCoef * 16.0 * a) as Coordinate,
      };
      arcCenters[10] = {
        x: (-25.0 * a) as Coordinate,
        y: (clockwiseCoef * -39.0 * a) as Coordinate,
      };
      ...
      rayEnd = {
        x: (directionPoint.x + bigNumber) as Coordinate,
        y: directionPoint.y as Coordinate,
      };
    }
...
class FibSpiralPriceAxisPaneView extends FibChannelAxisPaneView {
  getPoints(): [Coordinate | null, Coordinate | null] {
    ...
    const y1 = series.priceToCoordinate(
      this._source._bounds._minPrice as Coordinate
    );
    const y2 = series.priceToCoordinate(
      this._source._bounds._maxPrice as Coordinate
    );
    return [y1, y2];
  }
}
